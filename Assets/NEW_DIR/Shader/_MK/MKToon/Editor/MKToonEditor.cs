    ...
    public class MKToonEditor : ShaderGUI
    {
       ...
       //[MenuItem("CONTEXT/Material/Reset", false, 2100)]
        ...
        private void LightTypePopup(MaterialEditor materialEditor)
        {
            ...
            LightStyle bs = new LightStyle();
            bs = (LightStyle)lightType.floatValue;
            EditorGUI.BeginChangeCheck();
            bs = (LightStyle)EditorGUILayout.EnumPopup(GUIContentCollection.lightType, bs);
            if (EditorGUI.EndChangeCheck())
            {
                lightType.floatValue = (float)bs;
        ...
        private void BlendModePopup(MaterialEditor materialEditor)
        {
            ...
            BlendStyle bs = new BlendStyle();
            bs = (BlendStyle)blendMode.floatValue;
            EditorGUI.BeginChangeCheck();
            bs = (BlendStyle)EditorGUILayout.EnumPopup(GUIContentCollection.blendMode, bs);
            if (EditorGUI.EndChangeCheck())
            {
                blendMode.floatValue = (float)bs;
                materialEditor.RegisterPropertyChangeUndo("Rendering Mode");
                foreach (Material mat in blendMode.targets)
                {
                    SetBlendMode((Material)mat, (BlendStyle)blendMode.floatValue);
                }
            }
            EditorGUI.showMixedValue = false;
        }

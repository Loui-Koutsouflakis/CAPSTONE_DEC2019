Shader "Custom/Enemy_Toon_Shader"
{
	Properties
	{
		...
		_Bumpiness("Bumpiness",  Color) = (1,1,1,1)
				...
				SubShader
			{
				...
				float4 _Emission_ST;
					...
					v2fT vertT(appdataT v)
					{
						...
						o.emissive = TRANSFORM_TEX(v.e, _Emission);
						...
						float4 fragT(v2fT i) : SV_Target
						{
							...
							float4 normColor = _Bumpiness;
							float4 rim = rimIntensity * _RimColor;
							return tex2D(_MainTex, i.uv) * (tex2D(_NormalMap, i.worldNormal *0.5+0.5) * _Bumpiness) * tex2D(_Ramp, i.uvRamp) * (_AmbientColor + light + specular + rim) /* * i.diff*/;
						}
						ENDCG
					}
